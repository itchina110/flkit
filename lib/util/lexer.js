...
module.exports = think.Class(base, {
   ...
   */
  getQuote: function(chr, options){
    if (think.isObject(chr)) {
      options = chr;
      chr = '';
    }
    options = options || {};
    ...
    while(true){
      ...
      if (code === quoteCode) {
        if (!options.checkNext || this.text.charCodeAt(this.pos) !== code) {
          find = true;
          ret += chr;
          break;
        }
  ...
  getMatchedChar: function(startCode, endCode, options){
    ...
    var ret = this.forward(1), chr = this.text[this.pos];
