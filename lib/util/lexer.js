...
module.exports = think.Class(base, {
  ...
  /**
   * get matched string
   * @param  {String} start         [find start string]
   * @param  {String} end           [find end string]
   * @param  {Boolean} nestLoop []
   * @return {String}               [matched string]
   */
  getMatched1: function(start, end, nestLoop){
    if (!this.lookAt(start)) {
      return false;
    }
    var startLength = start.length, endLength = end.length;
    ...
    if (nestLoop && substr.indexOf(start) > -1) {
      ...
      ret = this.text.substr(startPos, this.pos - startPos);
    }else{
  ...
  getLineComment: function(){
    ...
    var i = 0, chr, ret = this.forward(2);
    for(;chr = this.next();){
      ret += chr;
      if (chr === '\n') {
        break;
