...
module.exports = think.Class(base, {
  ...
  compressCommon: function(token){
    ...
    if (token.type === TOKEN.HTML_DOCTYPE || 
        token.type === TOKEN.HTML_TEXT || 
        token.type === TOKEN.IE_HACK || 
        token.type === TOKEN.TPL) {
      return ret;
    }
    ...
    // space before end tag and prev token is script or style
    if (token.type === TOKEN.HTML_TAG_END && this.prevToken) {
  ...
  compressText: function(token){
    ...
    if (value === ' ' && this.nextToken && this.nextToken.type === TOKEN.HTML_DOCTYPE) {
      return '';
    }
    return value;
  },
