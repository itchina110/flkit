...
module.exports = think.Class(base, {
  ...
  compressEndTag: function(token){
    var value = token.value.toLowerCase(), options = this.options;
    ...
    if (where.next_comment === false) {
      if (this.nextToken && this.nextToken.comments.length && !options.remove_comment) {
        return value;
      }
    }
  ...
  compress: function(){
    ...
    for(;token = this.getToken();){
      output += this.compressComment(token);
      switch(token.type){
