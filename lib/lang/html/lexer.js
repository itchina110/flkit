...
module.exports = think.Class(lexer, {
    ...
    if (this.options.tag_attrs) {
      switch(type){
        case TOKEN.HTML_TAG_END:
          extra = {
            tag: ret.slice(2, -1).toLowerCase()
          }
          break;
        case TOKEN.HTML_TAG_START:
          extra = this.getTagAttrs(ret);
          break;
      }
    }
    return this.getToken(type, ret, extra);
  },
  /**
   * get tag attrs
  ...
  getTagAttrs: function(tag){
    tag = tag.slice(1, -1); //remove start < and end >
    var instance = this.getInstance(tag);
  },
  /**
   * skip comment
