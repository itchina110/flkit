      ...
      if (parseTag) {
        ...
        if (tplToken) {
          ...
          if (tplInstance.hasOutput(tplToken)) {
            ...
            if (hasEqual) {
              if (attrValue) {
                attrs.push([attrName, '=', attrValue], [tplToken.value]);
              }else{
                attrs.push([attrName, '=', tplToken.value]);
              }
              attrName = attrValue = '';
              hasEqual = false;
            }else{
        ...
        // char is =
        if (code === 0x3d) {
          var text = this.text.substr(this.pos);
          ...
          if (!hasEqual) {
            this.error('can\'t find = in tag', true);
          }
