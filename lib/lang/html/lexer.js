...
module.exports = think.Class(lexer, {
  ...
  getTagToken: function(){
    this.record();
    var ret = this.next();
    var type, tagEnd = false, chr, escape = false;
    /*jshint -W084 */
    for(;chr = this.next();){
  ...
  getTagAttrs: function(chr){
    ...
    var attrName = '', attrValue = '', escape = false;
    while(true){
      //parse tag name
      if (parseTag) {
        chr = this.next();
  ...
  skipComment: function(){
    ...
    commentLabel: while(this.text.charCodeAt(this.pos) === 0x3c && this.text.charCodeAt(this.pos + 1) === 0x21){
      ...
      var comment = this.getCommentToken('html');
      if (comment) {
  ...
  getRawToken: function(){
    var i = 0, item, pos = 0, code, startToken, contentToken, endToken;
    /*jshint -W084 */
    for(;item = rawTokens[i++];){
      ...
      return this.getToken(item[2], startToken.value + contentToken.value + endToken.value, {
        start: startToken,
